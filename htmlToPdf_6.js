...
async function generatePdf(htmlContent) {
    ...
    // Function to extract image data from an <img> element
    function extractImageData(imgElement) {
        // Regular expression to match the src attribute of the img element
        const imageRegex = /<img src="data:image\/([a-zA-Z]*);base64,([^\"]+)"[^>]*>/;
    
        // Match the regex against the img element string
        const match = imgElement.match(imageRegex);
    
        if (match) {
            // Extract the image type and base64 data
            const imageType = match[1]; // e.g., "png", "jpeg"
            const base64Data = match[2]; // The base64-encoded image data
    ...
    for (const match of matches) {
        ...
        if (tagOrText.startsWith('<img')) {
            // Process image
            const imageData = extractImageData(tagOrText);
            if (imageData) {
                const [imageType, base64Data] = imageData;
